...
namespace uicore
{
	...
	public:
		...
		static std::shared_ptr<Texture1DArray> create(const std::shared_ptr<GraphicContext> &context, int size, int array_size, TextureFormat texture_format = tf_rgba8, int levels = 1);
		...
		/// \param image Image to upload.
		/// \param level Mipmap level-of-detail number.
		virtual void set_subimage(const std::shared_ptr<GraphicContext> &context, int array_index, int x, const std::shared_ptr<PixelBuffer> &image, const int src_x, const int src_width, int level = 0) = 0;
