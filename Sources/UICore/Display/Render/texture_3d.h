...
namespace uicore
{
	...
	public:
		...
		static std::shared_ptr<Texture3D> create(const std::shared_ptr<GraphicContext> &context, int width, int height, int depth, TextureFormat texture_format = tf_rgba8, int levels = 1);
		static std::shared_ptr<Texture3D> create(const std::shared_ptr<GraphicContext> &context, const Vec3i &size, TextureFormat texture_format = tf_rgba8, int levels = 1);
		...
		virtual void set_image(const std::shared_ptr<GraphicContext> &context, const std::shared_ptr<PixelBuffer> &image, int depth, int level = 0) = 0;
		...
		/// \param image Image to upload.
		/// \param level Mipmap level-of-detail number.
		virtual void set_subimage(const std::shared_ptr<GraphicContext> &context, int x, int y, int z, const std::shared_ptr<PixelBuffer> &image, const Rect &src_rect, int level = 0) = 0;
