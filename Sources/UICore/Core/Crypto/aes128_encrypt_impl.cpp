...
namespace uicore
{
	AES128_Encrypt_Impl::AES128_Encrypt_Impl() : initialisation_vector_set(false), cipher_key_set(false), padding_enabled(true), padding_pkcs7(true), padding_num_additional_padded_blocks(0)
	{
		reset();
	}

	DataBufferPtr AES128_Encrypt_Impl::get_data() const
	{
		return databuffer;
	}

	void AES128_Encrypt_Impl::reset()
	{
		calculated = false;
	...
	void AES128_Encrypt_Impl::set_iv(const unsigned char iv[16])
	{
	...
	void AES128_Encrypt_Impl::set_padding(bool value, bool use_pkcs7, unsigned int num_additional_padded_blocks)
	{
	...
	void AES128_Encrypt_Impl::set_key(const unsigned char key[16])
	{
		cipher_key_set = true;
		extract_encrypt_key128(key, key_expanded);
	}
	...
	void AES128_Encrypt_Impl::calculate()
	{
		if (calculated)
			reset();
		...
		if (padding_enabled)
		{
			// Note, chunk_filled is never equal to aes128_block_size_bytes at this point, so we can process an empty block
			if (padding_pkcs7)
			{
				// PKCS#7
				unsigned char pad_size = aes128_block_size_bytes - chunk_filled;
			...
			else
			{
				...
				unsigned int pad_size = aes128_block_size_bytes - chunk_filled;
	...
	void AES128_Encrypt_Impl::process_chunk()
	{
		...
		*/
		...
		key_expanded_ptr += aes128_num_rounds_nr * 4;
