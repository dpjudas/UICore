...
namespace uicore
{
	AES192_Encrypt_Impl::AES192_Encrypt_Impl() : initialisation_vector_set(false), cipher_key_set(false), padding_enabled(true), padding_pkcs7(true), padding_num_additional_padded_blocks(0)
	{
		reset();
	}

	std::shared_ptr<DataBuffer> AES192_Encrypt_Impl::data() const
	{
		return databuffer;
	}

	void AES192_Encrypt_Impl::reset()
	{
		calculated = false;
	...
	void AES192_Encrypt_Impl::set_padding(bool value, bool use_pkcs7, unsigned int num_additional_padded_blocks)
	{
	...
	void AES192_Encrypt_Impl::set_key(const unsigned char key[24])
	{
		cipher_key_set = true;
		extract_encrypt_key192(key, key_expanded);
	}

	void AES192_Encrypt_Impl::add(const void *_data, int size)
	{
		if (calculated)
			reset();
