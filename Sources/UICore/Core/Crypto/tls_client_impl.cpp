...
namespace uicore
{
	...
	const void *TLSClient_Impl::decrypted_data() const
	{
		return recv_out_data->data() + recv_out_data_read_pos;
	}

	int TLSClient_Impl::decrypted_data_available() const
	{
		return recv_out_data->size() - recv_out_data_read_pos;
	}

	const void *TLSClient_Impl::encrypted_data() const
	{
		return send_out_data->data() + send_out_data_read_pos;
	}

	int TLSClient_Impl::encrypted_data_available() const
	{
	...
	bool TLSClient_Impl::send_application_data()
	{
		...
		auto message = Secret::create(offset);
	...
	void TLSClient_Impl::handshake_server_hello_received(const void *data, int size)
	{
		...
		// Get TLS_Random
		copy_data(security_parameters.server_random->data(), security_parameters.server_random->size(), data, size);

		// "This structure is generated by the server and must be different from (and independent of) ClientHello.random"
		if (!memcmp(security_parameters.server_random->data(), security_parameters.client_random->data(), security_parameters.server_random->size()))
			throw Exception("TLS Client and Server random numbers must not match");

		uint8_t session_id_length;
		copy_data(&session_id_length, 1, data, size);
		auto session_id = Secret::create(session_id_length);
		copy_data(session_id->data(), session_id_length, data, size);
	...
	void TLSClient_Impl::handshake_finished_received(const void *data, int size)
	{
		...
		auto server_verify_data = Secret::create(verify_data_size);
		copy_data(server_verify_data->data(), verify_data_size, data, size);

		auto client_verify_data = Secret::create(verify_data_size);

		auto md5_handshake_messages = Secret::create(16);
		auto sha1_handshake_messages = Secret::create(20);

		server_handshake_md5_hash->calculate();
		server_handshake_sha1_hash->calculate();

		server_handshake_md5_hash->hash(md5_handshake_messages->data());
		server_handshake_sha1_hash->hash(sha1_handshake_messages->data());

		PRF(client_verify_data->data(), verify_data_size, security_parameters.master_secret, "server finished", md5_handshake_messages, sha1_handshake_messages);

		if (memcmp(client_verify_data->data(), server_verify_data->data(), verify_data_size))
	...
	{
		unsigned char *key_ptr = security_parameters.client_random->data();
		m_Random->random_bytes(key_ptr + 4, 28);
	...
	bool TLSClient_Impl::send_client_hello()
	{
		...
		auto message = Secret::create(offset);	// keep data secure
		unsigned char *message_ptr = message->data();
	...
	bool TLSClient_Impl::send_client_key_exchange()
	{
		...
		// itself.
		auto pre_master_secret = Secret::create(48);
		unsigned char *pms_ptr = pre_master_secret->data();
		m_Random->random_bytes(pms_ptr + 2, 46);
		...
		auto key_block = Secret::create(2 * (security_parameters.hash_size + security_parameters.key_material_length + security_parameters.iv_size));
		PRF(key_block->data(), key_block->size(), security_parameters.master_secret, "key expansion", security_parameters.server_random, security_parameters.client_random);

		security_parameters.client_write_mac_secret = Secret::create(security_parameters.hash_size);
		security_parameters.server_write_mac_secret = Secret::create(security_parameters.hash_size);
		security_parameters.client_write_key = Secret::create(security_parameters.key_material_length);
		security_parameters.server_write_key = Secret::create(security_parameters.key_material_length);
		security_parameters.client_write_iv = Secret::create(security_parameters.iv_size);
		security_parameters.server_write_iv = Secret::create(security_parameters.iv_size);

		unsigned char *key_block_ptr = key_block->data();

		memcpy(security_parameters.client_write_mac_secret->data(), key_block_ptr, security_parameters.client_write_mac_secret->size());
		key_block_ptr+=security_parameters.client_write_mac_secret->size();

		memcpy(security_parameters.server_write_mac_secret->data(), key_block_ptr, security_parameters.server_write_mac_secret->size());
		key_block_ptr+=security_parameters.server_write_mac_secret->size();

		memcpy(security_parameters.client_write_key->data(), key_block_ptr, security_parameters.client_write_key->size());
		key_block_ptr+=security_parameters.client_write_key->size();

		memcpy(security_parameters.server_write_key->data(), key_block_ptr, security_parameters.server_write_key->size());
		key_block_ptr+=security_parameters.server_write_key->size();

		memcpy(security_parameters.client_write_iv->data(), key_block_ptr, security_parameters.client_write_iv->size());
		key_block_ptr+=security_parameters.client_write_iv->size();

		memcpy(security_parameters.server_write_iv->data(), key_block_ptr, security_parameters.server_write_iv->size());
		key_block_ptr+=security_parameters.server_write_iv->size();
	...
	void TLSClient_Impl::PRF(void *output_ptr, unsigned int output_size, const SecretPtr &secret, const char *label_ptr, const SecretPtr &seed_part1, const SecretPtr &seed_part2)
	{
		...
		auto md5_output_a = Secret::create(MD5::hash_size);
		auto md5_output_b = Secret::create(MD5::hash_size);
		auto sha1_output_a = Secret::create(SHA1::hash_size);
		auto sha1_output_b = Secret::create(SHA1::hash_size);

		auto md5 = MD5::create();
		md5->set_hmac(secret_part1, split_length);
		md5->add(label_ptr, label_length);
		md5->add(seed_part1->data(), seed_part1->size());
		md5->add(seed_part2->data(), seed_part2->size());
		md5->calculate();
		md5->hash(md5_output_a->data());

		auto sha1 = SHA1::create();
	
		sha1->set_hmac(secret_part2, split_length);
		sha1->add(label_ptr, label_length);
		sha1->add(seed_part1->data(), seed_part1->size());
		sha1->add(seed_part2->data(), seed_part2->size());
		sha1->calculate();
		sha1->hash(sha1_output_a->data());

		unsigned char *out_ptr = ( unsigned char *) output_ptr;
		unsigned char *md5_ptr = md5_output_b->data();
		unsigned char *sha1_ptr = sha1_output_b->data();
		...
		for (int cnt=0; cnt<output_size; cnt++)
		{
			...
			{
				md5->set_hmac(secret_part1, split_length);
				md5->add(md5_output_a->data(), md5_output_a->size());
				md5->add(label_ptr, label_length);
				md5->add(seed_part1->data(), seed_part1->size());
				md5->add(seed_part2->data(), seed_part2->size());
				md5->calculate();
			...
			{
				sha1->set_hmac(secret_part2, split_length);
				sha1->add(sha1_output_a->data(), sha1_output_a->size());
				sha1->add(label_ptr, label_length);
				sha1->add(seed_part1->data(), seed_part1->size());
				sha1->add(seed_part2->data(), seed_part2->size());
				sha1->calculate();
	...
	bool TLSClient_Impl::send_finished()
	{
		...
		auto md5_handshake_messages = Secret::create(MD5::hash_size);
		auto sha1_handshake_messages = Secret::create(SHA1::hash_size);

		client_handshake_md5_hash->calculate();
		client_handshake_sha1_hash->calculate();

		client_handshake_md5_hash->hash(md5_handshake_messages->data());
		client_handshake_sha1_hash->hash(sha1_handshake_messages->data());
	...
	DataBufferPtr TLSClient_Impl::encrypt_data(const void *data_ptr, unsigned int data_size, const void *mac_ptr, unsigned int mac_size)
	{
		...
		if (security_parameters.bulk_cipher_algorithm == cl_tls_cipher_algorithm_aes128)
		{
			...
			encrypt->set_iv(security_parameters.client_write_iv->data());
			encrypt->set_key(security_parameters.client_write_key->data());
		...
		{
			auto encrypt = AES256_Encrypt::create();
			encrypt->set_padding(true, false, additional_unpadded_blocks);
			encrypt->set_iv(security_parameters.client_write_iv->data());
			encrypt->set_key(security_parameters.client_write_key->data());
			encrypt->add(data_ptr, data_size);
		...
		memcpy(security_parameters.client_write_iv->data(), buffer->data() + buffer->size() - security_parameters.client_write_iv->size(), security_parameters.client_write_iv->size());
		return buffer;
	...
	SecretPtr TLSClient_Impl::calculate_mac(const void *data_ptr, unsigned int data_size, const void *data2_ptr, unsigned int data2_size, uint64_t sequence_number, const SecretPtr &mac_secret)
	{
		...
		{
			auto hash = Secret::create(20);
			auto sha1 = SHA1::create();
			sha1->set_hmac(mac_secret->data(), mac_secret->size());
		...
		{
			auto hash = Secret::create(32);
			auto sha256 = SHA256::create();
			sha256->set_hmac(mac_secret->data(), mac_secret->size());
	...
	DataBufferPtr TLSClient_Impl::decrypt_data(const void *data_ptr, unsigned int data_size)
	{
		...
		if (security_parameters.bulk_cipher_algorithm == cl_tls_cipher_algorithm_aes128)
		{
			...
			decrypt->set_iv(security_parameters.server_write_iv->data());
			decrypt->set_key(security_parameters.server_write_key->data());
		...
		{
			auto decrypt = AES256_Decrypt::create();
			decrypt->set_padding(true, false);
			decrypt->set_iv(security_parameters.server_write_iv->data());
			decrypt->set_key(security_parameters.server_write_key->data());
			decrypt->add(data_ptr, data_size);
		...
		last_block += data_size - security_parameters.server_write_iv->size();
		memcpy(security_parameters.server_write_iv->data(), last_block, security_parameters.server_write_iv->size());
		return buffer;
	...
	DataBufferPtr TLSClient_Impl::decrypt_record(TLS_Record &record, const DataBufferPtr &record_data)
	{
		...
		auto mac = calculate_mac(&record, sizeof(record), decrypted_data , decoded_size, security_parameters.read_sequence_number, security_parameters.server_write_mac_secret);	// MAC includes the header and sequence number

		if (memcmp(mac->data(), decrypted_data + decoded_size, mac->size()))
