...
namespace uicore
{
	AES128_Decrypt_Impl::AES128_Decrypt_Impl() : initialisation_vector_set(false), cipher_key_set(false), padding_enabled(true), padding_pkcs7(true)
	{
		reset();
	}

	std::shared_ptr<DataBuffer> AES128_Decrypt_Impl::data() const
	{
		return databuffer;
	}

	void AES128_Decrypt_Impl::reset()
	{
		calculated = false;
	...
	void AES128_Decrypt_Impl::set_iv(const unsigned char iv[16])
	{
	...
	void AES128_Decrypt_Impl::set_padding(bool value, bool use_pkcs7)
	{
		padding_enabled = value;
		padding_pkcs7 = use_pkcs7;
	}

	void AES128_Decrypt_Impl::set_key(const unsigned char key[16])
	{
		cipher_key_set = true;
		extract_encrypt_key128(key, key_expanded);
		extract_decrypt_key(key_expanded, aes128_num_rounds_nr);
	}

	void AES128_Decrypt_Impl::add(const void *_data, int size)
	{
		if (calculated)
			reset();
	...
	bool AES128_Decrypt_Impl::calculate()
	{
		if (calculated)
			reset();
		...
		if (padding_enabled)
		{
			if (chunk_filled == aes128_block_size_bytes)
			{
				...
				if (current_size > 0)
				{
					...
					if (padding_pkcs7)
					{
						if ((pad_byte == 0) || (pad_byte > aes128_block_size_bytes))
						{
