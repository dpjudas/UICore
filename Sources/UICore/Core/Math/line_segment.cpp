...
namespace clan
{
	...
	Vec2<Type> LineSegment2x<Type>::normal() const
	{
		Vec2<Type> N;
		N.x = -(q.y - p.y);
		N.y = q.x - p.x;
	...
	Vec2i LineSegment2x<int>::normal() const
	{
		...
		Vec2f n = line.normal();
		Vec2i vec((int)(n.x * 256.0f), (int)(n.y * 256.0f));
		return vec;
	}
	...
	Vec2<Type> LineSegment2x<Type>::get_intersection(const LineSegment2x<Type> &second, bool &intersect) const
	{
		...
		intersect = true;
		return Vec2<Type>(p.x + r*(q.x - p.x), p.y + r*(q.y - p.y));
	}
	...
	template<typename Type>
	Type LineSegment3x<Type>::point_distance(const Vec3<Type> &point, Vec3<Type> &dest_intercept) const
	{
		Vec3<Type> ap(point.x - p.x, point.y - p.y, point.z - p.z);
		Vec3<Type> ab(q.x - p.x, q.y - p.y, q.z - p.z);
	...
	int LineSegment3x<int>::point_distance(const Vec3i &point, Vec3i &dest_intercept) const
	{
		...
		Vec3i ab(q.x - p.x, q.y - p.y, q.z - p.z);
...
#define LINECLIP_TOP 1
#define LINECLIP_RIGHT 2
#define LINECLIP_BOTTOM 4
#define LINECLIP_LEFT 8

#define CREATE_REGION_CODE(point_x, point_y, region_code) \
	region_code = 0; \
	if (point_y < rect.top) region_code = LINECLIP_TOP; \
		else if (point_y > rect.bottom) region_code = LINECLIP_BOTTOM; \
	if (point_x > rect.right) region_code |= LINECLIP_RIGHT; \
		else if (point_x < rect.left) region_code |= LINECLIP_LEFT;

	template<typename Type>
	LineSegment2x<Type> &LineSegment2x<Type>::clip(const Rectx<Type> &rect, bool &clipped)
	{
		...
		CREATE_REGION_CODE(p.x, p.y, region_code_p);
		CREATE_REGION_CODE(q.x, q.y, region_code_q);
		...
		while (true)
		{
			...
			if (region_code_out & LINECLIP_TOP)			// point is above the clip rectangle
			{
				x = p.x + (q.x - p.x) * (rect.top - p.y) / (q.y - p.y);
				y = rect.top;
			}
			else if (region_code_out & LINECLIP_BOTTOM)	// point is below the clip rectangle
			{
				x = p.x + (q.x - p.x) * (rect.bottom - p.y) / (q.y - p.y);
				y = rect.bottom;
			}
			else if (region_code_out & LINECLIP_RIGHT)	// point is to the right of clip rectangle
			{
				y = p.y + (q.y - p.y) * (rect.right - p.x) / (q.x - p.x);
				x = rect.right;
			}
			else /* if (region_code_out & LINECLIP_LEFT) */	// point is to the left of clip rectangle
			{
			...
			if (region_code_out == region_code_p)
			{
				p.x = x;
				p.y = y;
				CREATE_REGION_CODE(x, y, region_code_p);
			}
			else
			{
				q.x = x;
				q.y = y;
				CREATE_REGION_CODE(x, y, region_code_q);
			}
