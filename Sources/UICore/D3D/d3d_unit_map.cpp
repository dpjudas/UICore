...
namespace uicore
{
	...
	void D3DUnitMap::bind_image(D3DGraphicContextProvider *gc, int index)
	{
		if (image_units.size() > index)
		{
			...
			if (image_units[index].shader_index[(int)ShaderType::compute] != -1)
			{
	...
	void D3DUnitMap::bind_storage_buffer(D3DGraphicContextProvider *gc, int index)
	{
		if (storage_units.size() > index)
		{
			...
			if (storage_units[index].shader_uav_index[(int)ShaderType::compute] != -1)
			{
				ID3D11UnorderedAccessView *uav = 0;
	...
	void D3DUnitMap::unbind_image(D3DGraphicContextProvider *gc, int index)
	{
		if (image_units.size() > index)
		{
			ID3D11UnorderedAccessView *uav = 0;
			UINT uav_initial_count = 0;
			if (texture_units[index].shader_index[(int)ShaderType::compute] != -1)
				gc->get_window()->get_device_context()->CSSetUnorderedAccessViews(texture_units[index].shader_index[(int)ShaderType::compute], 1, &uav, &uav_initial_count);
		}
	}
