...
namespace uicore
{
	D3DUniformBuffer::D3DUniformBuffer(const ComPtr<ID3D11Device> &device, int new_size, BufferUsage usage)
	{
	...
	D3DUniformBuffer::D3DUniformBuffer(const ComPtr<ID3D11Device> &device, const void *data, int new_size, BufferUsage usage)
	{
		handles.push_back(std::shared_ptr<DeviceHandles>(new DeviceHandles(device)));
	...
	void D3DUniformBuffer::upload_data(const std::shared_ptr<GraphicContext> &gc, const void *data, int data_size)
	{
		if (data_size != size)
	...
	void D3DUniformBuffer::copy_from(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<StagingBuffer> &buffer, int dest_pos, int src_pos, int copy_size)
	{
		auto d3d_window = static_cast<D3DGraphicContext*>(gc.get())->get_window();
		const auto &device = d3d_window->get_device();
		const auto &device_context = d3d_window->get_device_context();
	...
	void D3DUniformBuffer::copy_to(const std::shared_ptr<GraphicContext>  &gc, const std::shared_ptr<StagingBuffer> &buffer, int dest_pos, int src_pos, int copy_size)
	{
		auto d3d_window = static_cast<D3DGraphicContext*>(gc.get())->get_window();
		const auto &device = d3d_window->get_device();
		const auto &device_context = d3d_window->get_device_context();
	...
	void D3DUniformBuffer::device_destroyed(ID3D11Device *device)
	{
	...
	D3DUniformBuffer::DeviceHandles &D3DUniformBuffer::get_handles(const ComPtr<ID3D11Device> &device)
	{
		for (size_t i = 0; i < handles.size(); i++)
	...
	D3D11_MAP D3DUniformBuffer::to_d3d_map_type(BufferAccess access)
	{
		switch (access)
		{
