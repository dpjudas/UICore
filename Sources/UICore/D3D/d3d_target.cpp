...
namespace uicore
{
	...
	ID3D11Texture2D *D3DTarget::texture2d_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<Texture> &texture)
	{
	...
	ID3D11Texture2D *D3DTarget::texture2d_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<PixelBuffer> &pixel_buffer)
	{
	...
	ID3D11Texture2D *D3DTarget::texture2d_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<RenderBuffer> &render_buffer)
	{
	...
	ID3D11Buffer *D3DTarget::buffer_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<VertexArrayBuffer> &buffer)
	{
	...
	ID3D11Buffer *D3DTarget::buffer_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<ElementArrayBuffer> &buffer)
	{
		D3DElementArrayBuffer *provider = static_cast<D3DElementArrayBuffer *>(buffer.get());
		return provider->get_buffer(static_cast<const D3DGraphicContext *>(gc.get())->get_window()->get_device());
	}

	ID3D11Buffer *D3DTarget::buffer_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<UniformBuffer> &buffer)
	{
		D3DUniformBuffer *provider = static_cast<D3DUniformBuffer *>(buffer.get());
		return provider->get_buffer(static_cast<const D3DGraphicContext *>(gc.get())->get_window()->get_device());
	}

	ID3D11ShaderResourceView *D3DTarget::srv_handle(const std::shared_ptr<GraphicContext> &gc, const std::shared_ptr<Texture> &texture)
	{
		D3DTextureObject *provider = static_cast<D3DTextureObject *>(texture->texture_object());
		return provider->get_srv(static_cast<const D3DGraphicContext *>(gc.get())->get_window()->get_device());
	}

	ID3D11Device *D3DTarget::device_handle(const std::shared_ptr<GraphicContext>  &gc)
	{
		const D3DGraphicContext *provider = static_cast<const D3DGraphicContext *>(gc.get());
		return provider->get_window()->get_device();
	}

	ID3D11DeviceContext *D3DTarget::device_context_handle(const std::shared_ptr<GraphicContext> &gc)
	{
		const D3DGraphicContext *provider = static_cast<const D3DGraphicContext *>(gc.get());
		return provider->get_window()->get_device_context();
	}

	IDXGISwapChain *D3DTarget::swap_chain_handle(const std::shared_ptr<GraphicContext> &gc)
	{
		const D3DGraphicContext *provider = static_cast<const D3DGraphicContext *>(gc.get());
		return provider->get_window()->get_swap_chain();
	}

	ID3D11RenderTargetView *D3DTarget::back_buffer_rtv_handle(const std::shared_ptr<GraphicContext> &gc)
	{
		const D3DGraphicContext *provider = static_cast<const D3DGraphicContext *>(gc.get());
