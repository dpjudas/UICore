...
namespace uicore
{
	...
	public:
		...
		static std::shared_ptr<Texture1D> create(GraphicContext &context, const PixelBufferPtr &image, bool is_srgb = false);
		static std::shared_ptr<Texture1D> create(GraphicContext &context, const PixelBufferPtr &image, int src_x, int width, bool is_srgb = false);
		...
		/// \param image Image to upload.
		/// \param level Mipmap level-of-detail number.
		virtual void set_subimage(GraphicContext &context, int x, const PixelBufferPtr &image, const int src_x, const int src_width, int level = 0) = 0;
